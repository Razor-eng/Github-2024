...
import { GoGitPullRequest, GoMilestone } from "react-icons/go"
import { HiOutlineTag } from "react-icons/hi2"
import { IoBulbOutline } from "react-icons/io5"
import { RiArrowDownSFill } from "react-icons/ri"

const PullRequests = ({ currentUser }) => {
    return (
        <div className='pt-[24px] px-[150px]'>
            ...
            <div className="mt-4 mb-4 w-full">
                <div className="w-full flex">
                    ...
                    <button className="dark:text-zinc-200 text-sm font-semibold py-1 px-2 bg-[#29903B] hover:opacity-90 ml-4 rounded-md">New pull request</button>
                </div>
            </div>
            <div className="border rounded-md w-full dark:text-white flex border-zinc-300 dark:border-zinc-800 justify-center">
                <div className="max-w-[768px] min-h-[320px] py-[40px] px-[80px] flex gap-5 flex-col justify-center items-center">
                    <GoGitPullRequest size={30} color="#848D97" />
                    <h2 className="text-2xl font-semibold">Welcome to issues!</h2>
                    <p className="dark:text-[#848D97] text-sm">Pull requests help you collaborate on code with other people. As pull requests are created, they’ll appear here in a searchable and filterable list. To get started, you should <span className="text-[#4493F8] hover:underline">create a pull request</span>.</p>
                </div>
            </div>
            <div className="dark:text-white flex w-full justify-center gap-1 mt-4">
                <IoBulbOutline color="#848D97" size={20} />
                <h2 className="font-bold">ProTip! <span className="text-[#848D97] font-normal">Ears burning? Get @{currentUser?.name} mentions with</span> <span className="text-[#1F6FEB] underline">mentions:{currentUser?.name}</span>.</h2>
            </div>
        </div >
    )
}
